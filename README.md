...
Note for Save Settings:  
  Sometimes Streamlabs Chatbot doesn't trigger `ReloadSettings` method on
    new settings save from Save Settings GUI button.

  If you can't hear TTS say the `Configuration updated successfully`
    sentence (by default `Configuration updated successfully`) probably new
...
* Description: It greets new viewers by playing a sound and/or a text
      ...
      time each session.
    It can also be used to play sounds when users send additional messages.
...
Follow this tutorial to prepare your Streamlabs Chatbot installation to accept
  scripts.
...
If your chat uses often messages starting by some characters or words,
  ...
  by listing sequences of characters, and seaparte each sequence with a
  space.
